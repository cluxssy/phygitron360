...
export default function MyProfile() {
  ...
  return (
    <div className="space-y-6 max-w-5xl animate-fade-in-up pb-20">
      {/* Action Bar */}
      <div className="flex justify-between items-center bg-white/5 p-4 rounded-2xl border border-white/5">
        <h2 className="text-[10px] font-black uppercase tracking-[0.3em] text-white/40 italic">Personnel Dossier // Self Management</h2>
        <div className="flex gap-3">
            <button 
                onClick={() => setShowPwdModal(true)}
                className="flex items-center gap-2 px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white/40 hover:text-white transition-all"
            >
                <Key size={14} /> Update Key
            </button>
            {editMode ? (
                <>
                    <button onClick={() => setEditMode(false)} className="px-6 py-2 text-[10px] font-black uppercase tracking-widest text-white/40">Abort</button>
                    <button 
                        onClick={handleSave} 
                        disabled={isSaving}
      ...
      {/* Profile Hero */}
      <div className="glass-panel p-10 border-white/5 flex flex-col md:flex-row items-start gap-8 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-primary/5 blur-[100px] rounded-full -translate-y-1/2 translate-x-1/2" />
        
        <div className="relative group">
            <div className="w-24 h-24 rounded-3xl bg-primary/10 border border-primary/20 flex items-center justify-center text-primary font-display font-black text-4xl shrink-0 shadow-[0_0_30px_rgba(204,151,255,0.2)] overflow-hidden">
            {details.photo_path ? (
            ...
            {editMode && (
                <button 
                    ...
                    className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-center gap-1 rounded-3xl text-white text-[8px] font-black uppercase tracking-widest"
                >
            ...
            <div className="flex items-center gap-3 mb-1">
                ...
                {editMode && (
                    <input 
                        ...
                        className="bg-white/5 border border-white/10 rounded-lg px-2 py-0.5 text-[10px] text-white/40 focus:outline-none"
                    />
                )}
            </div>
            {editMode ? (
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              <div className="glass-panel p-8 border-white/5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-6">
                    <div>
                        <p className="text-[9px] font-black uppercase tracking-widest text-primary mb-3">Primary Skills</p>
                        {editMode ? (
                            ...
                            <div className="flex flex-wrap gap-2">
                                {(details.skill_matrix?.primary_skillset || '').split(',').map((s, i) => (
                                    <span key={i} className="px-2 py-1 bg-primary/10 text-primary text-[9px] font-bold uppercase rounded-lg">
                                        {s.trim()}
                                    </span>
                                ))}
                            </div>
                    ...
                    <div>
                        <p className="text-[9px] font-black uppercase tracking-widest text-white/30 mb-3">Secondary Skills</p>
                        {editMode ? (
                            ...
                            <div className="flex flex-wrap gap-2">
                                {(details.skill_matrix?.secondary_skillset || '').split(',').map((s, i) => (
                                    <span key={i} className="px-2 py-1 bg-white/5 text-white/50 text-[9px] font-bold uppercase rounded-lg">
                                        {s.trim()}
                                    </span>
                                ))}
                            </div>
    ...
    return (
        <div className="space-y-1">
            ...
            {editMode ? (
                ...
                <p className="text-xs text-white/70 font-bold truncate">{value || '—'}</p>
            )}
        </div>
    );
}
    ...
    return (
        <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5 group hover:border-primary/30 transition-all">
            <div className="flex items-center gap-3">
                <FileText size={16} className="text-white/20 group-hover:text-primary" />
                <span className="text-[10px] font-black uppercase tracking-widest text-white/40 group-hover:text-white/80">{label}</span>
            </div>
            <div className="flex gap-2">
                {path && (
                    <a href={`/${path}`} target="_blank" rel="noreferrer" className="p-2 bg-white/5 rounded-lg text-primary hover:bg-primary hover:text-black">
                        <ExternalLink size={12} />
                    </a>
                )}
                {editMode && (
                    <button onClick={onUpload} className="p-2 bg-white/5 rounded-lg text-white/40 hover:text-white transition-all">
                        <Upload size={12} />
                    </button>
                )}
            </div>
